...
- `scale_advances.py <bundle> <out> <factor> [ассеты]` — рабочий способ:
  ...
  коэффициент **0.85** (−15%; в v0.0.2 был 0.90, по отзывам сжато сильнее).
  Меньше 0.85 — засечки начинают касаться, ниже не опускаться.
...
6. `bake_cyrillic.py` — повторный рендер глифов из новых оригиналов
   ...
   нет). После него заново примените `scale_advances.py` (×0.85).
