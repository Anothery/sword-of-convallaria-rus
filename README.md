...
## Дополнительные прогоны (v0.0.4)

- **`conv_fix.py queue|run|apply`** — диалоги доверия (вторая схема
  шаблона `name`+`text`): LLM размечает реплики npc/player и правит
  гендер говорящего и адресата (204 правки).
- **`conv2_fix.py`** — story-строки `text` без пометки спикера: заголовки
  сцен из `lua_dbtemplateperformance` (там имя персонажа) + LLM с правилом
  «не уверен — нейтрализуй» (25 правок).
- **`scen_fix.py`** — `scenario_text` (23,7 тыс. строк): привязки спикеров
  нет, LLM работает по контексту соседних строк. Урок: свободный промпт
  угадывает пол и портит верное (112 f→m флипов) — только строгий
  асимметричный (m→f по явным доказательствам, 173 правки).
- **`style_full.py queue|run|apply`** — стилистический прогон всей TM на
  естественность русского (13 343 правки, 317 отклонено валидацией).
  deepinfra/deepseek-v4-flash-0731 держит 200 воркеров (~21 тыс.
  строк/мин) — самый быстрый провайдер для таких прогонов.
